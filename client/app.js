...
function saveProfile(session) {
    ...
    try {
        ...
        if (currentProfile.user?.username)
            localStorage.setItem("xiangqi.nickname", currentProfile.user.username);
    }
    catch { /* ignore */ }
    renderProfile();
}
...
function addIfValid(moves, r, c, color, targetBoard = board) {
    if (!inBounds(r, c))
        return;
    if (colorOf(targetBoard[r][c]) !== color)
        moves.push({ r, c });
}
function pseudoMoves(fromR, fromC, targetBoard = board) {
    ...
    if (kind === "k") {
        for (const [dr, dc] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
            ...
            const c = fromC + dc;
            if (palace(color, r, c))
                addIfValid(moves, r, c, color, targetBoard);
        }
        const step = color === "red" ? -1 : 1;
        for (let r = fromR + step; inBounds(r, fromC); r += step) {
            const seen = targetBoard[r][fromC];
            if (seen) {
                if (seen.toLowerCase() === "k" && colorOf(seen) !== color)
                    moves.push({ r, c: fromC });
                break;
            }
        }
    }
    ...
    if (kind === "b") {
        for (const [dr, dc] of [[2, 2], [2, -2], [-2, 2], [-2, -2]]) {
            ...
            if (inBounds(r, c) && staysHome && !targetBoard[eyeR][eyeC])
                addIfValid(moves, r, c, color, targetBoard);
        }
    }
    if (kind === "n") {
        for (const move of [
            ...
            const r = fromR + dr;
            const c = fromC + dc;
            if (inBounds(r, c) && !targetBoard[fromR + legR][fromC + legC])
                addIfValid(moves, r, c, color, targetBoard);
        }
    }
    if (kind === "r" || kind === "c") {
...
function isInCheck(color, targetBoard = board) {
    ...
    for (let r = 0; r < 10; r += 1) {
        for (let c = 0; c < 9; c += 1) {
            if (targetBoard[r][c]?.toLowerCase() === "k" && colorOf(targetBoard[r][c]) === color)
                king = { r, c };
        }
    }
    if (!king || kingsFacing(targetBoard))
        return true;
...
function buildPgn() {
    ...
    if (gameResult?.reason || online?.winReason)
        headers.push(["Termination", gameResult?.reason || online.winReason]);
    ...
    for (let i = 0; i < history.length; i += 2) {
        ...
        let token = `${moveNo}.`;
        if (red)
            token += ` ${recordToUci(red)}`;
        if (black)
            token += ` ${recordToUci(black)}`;
        tokens.push(token);
    }
    tokens.push(result);
    return `${lines.join("\n")}\n\n${tokens.join(" ")}\n`;
...
function shouldFlipBoard() {
    if (modeSelect.value === "online" && online && online.myColor === "black")
        return true;
    if (modeSelect.value === "ai" && playerSide.value === "black")
        return true;
    if (modeSelect.value === "trainer" && playerSide.value === "black")
        return true;
    return false;
}
  ...
  `;
    ...
    for (let r = 0; r < 10; r += 1) {
        for (let c = 0; c < 9; c += 1) {
            ...
            if (legalSet.has(`${r},${c}`))
                square.classList.add(board[r][c] ? "capture" : "hint");
            if (trainerMove && trainerState.hintLevel >= 1 && trainerMove.fromR === r && trainerMove.fromC === c)
                square.classList.add("trainer-source");
            if (trainerMove && trainerState.hintLevel >= 2 && trainerMove.toR === r && trainerMove.toC === c)
                square.classList.add("trainer-target");
            const piece = board[r][c];
            if (piece) {
...
function canHumanMove() {
    ...
    if (modeSelect.value === "online") {
        if (!online || online.role !== "player" || online.status !== "playing")
            return false;
...
function clockSnapshot(color) {
    if (!clock)
        return { baseLeft: CLOCK_BASE_MS, moveLeft: CLOCK_OPENING_MS };
...
function clearClock() {
    ...
    for (const el of [clockRowRed, clockRowBlack])
        el.classList.remove("active", "low", "flagged");
}
...
function gradePlayedMove(moves, mover, playedUci, playedScore = null) {
    ...
    if (!best?.move)
        return { quality: "unknown", rank: playedIndex + 1, lossCp: null };
    if (playedUci === best.move)
        return { quality: "best", rank: 1, lossCp: 0 };
...
function moveQualityText(grade) {
    if (grade.quality === "best")
        return "Best move.";
    if (grade.quality === "good")
        return `Good move: lost ${cpLossText(grade.lossCp)}.`;
    if (grade.lossCp != null)
        return `Needs work: lost ${cpLossText(grade.lossCp)}.`;
    return "";
}
...
function triggerPreMoveAnalysis() {
    if (modeSelect.value === "pgn" || modeSelect.value === "trainer")
        return; // PGN/trainer modes have their own analysis flow
    const fen = boardToFen();
    preMoveActiveFen = fen;
    if (preMoveAnalysisCache.has(fen))
        return;
...
async function enginePost(path, data) {
    ...
    if (api) {
        if (path === "/api/evaluate")
            return api.evaluate(data);
        if (path === "/api/bestmove")
            return api.bestmove(data);
        if (path === "/api/topmoves")
            return api.topmoves(data);
...
async function maybeAiMove() {
    if (modeSelect.value === "online" || modeSelect.value === "pgn")
        return;
    if (modeSelect.value !== "ai" || playerSide.value === turn)
        return;
    busy = true;
...
function isLegalParsedMove(move) {
    if (!move || !inBounds(move.fromR, move.fromC) || !inBounds(move.toR, move.toC))
        return false;
...
function trainerPromptText() {
    ...
    if (turn !== playerSide.value)
        return `${colorLabel(turn)} reply is being played.`;
    if (trainerState.status === "loading")
        return `Preparing ${colorLabel(turn)} position.`;
    if (trainerState.status === "error")
        return "Trainer is unavailable for this position.";
    return `Find ${colorLabel(turn)}'s best move.`;
}
function trainerLineText() {
    if (trainerState.status === "error")
        return trainerState.message || "Trainer unavailable.";
    if (trainerState.status === "loading")
        return "Preparing position.";
    if (turn !== playerSide.value)
        return `${colorLabel(turn)} is Pikafish's side.`;
    if (trainerState.revealed)
        return trainerTopMoveLines();
    if (trainerState.hintLevel >= 1 && trainerState.bestMove) {
        const move = uciToMove(trainerState.bestMove);
        const piece = board[move?.fromR]?.[move?.fromC];
        if (move && piece)
            return `Hint: ${pieceNames[piece]} from ${squareToUci(move.fromR, move.fromC)}.`;
    }
...
async function prepareTrainerChallenge() {
    if (modeSelect.value !== "trainer" || gameResult)
        return;
    const fen = boardToFen();
    if (turn !== playerSide.value) {
        trainerState = {
    ...
    try {
        ...
        if (requestId !== trainerState.requestId || modeSelect.value !== "trainer" || fen !== boardToFen())
            return;
        ...
        if (requestId !== trainerState.requestId || modeSelect.value !== "trainer")
            return;
        trainerState = {
            ...trainerState,
            status: "error",
...
async function trainerMaybeOpponentMove() {
    ...
    try {
        ...
        if (modeSelect.value !== "trainer" || fen !== boardToFen())
            return;
        const move = uciToMove(result.bestMove);
        if (!isLegalParsedMove(move))
            throw new Error("Pikafish returned no legal reply.");
    ...
    if (!failed && !ended && modeSelect.value === "trainer")
        await prepareTrainerChallenge();
}
...
async function submitTrainerMove(fromR, fromC, toR, toC) {
    if (modeSelect.value !== "trainer" || busy)
        return;
    const fen = boardToFen();
...
function showTrainerHint() {
    if (modeSelect.value !== "trainer" || trainerState.status !== "ready" || !trainerState.bestMove)
        return;
...
async function getServerUrl() {
    ...
    if (window.api?.serverUrl) {
        try {
            cachedServerUrl = await window.api.serverUrl();
        }
        catch {
            cachedServerUrl = "";
        }
...
function hasAnyLegalMove() {
    for (let r = 0; r < 10; r += 1) {
        for (let c = 0; c < 9; c += 1) {
            if (colorOf(board[r][c]) === turn && legalMoves(r, c).length > 0)
                return true;
        }
    }
    return false;
...
function onlineSavedNickname() {
    try {
        return localStorage.getItem("xiangqi.nickname") || currentProfile.user?.username || "";
    }
    catch {
        return currentProfile.user?.username || "";
    }
}
function onlineSaveNickname(value) {
    try {
        localStorage.setItem("xiangqi.nickname", value);
    }
    catch { /* ignore */ }
}
...
function requireLoginForMatch() {
    if (currentProfile.token && currentProfile.user)
        return true;
...
async function onlineQueue() {
    ...
    try {
        ...
        enterRoom(result);
        if (!result.matched)
            onlineLobbyStatus.textContent = `Waiting for an opponent. Share code ${result.code} to invite a friend.`;
    }
    catch (e) {
        onlineLobbyStatus.textContent = e.message;
    }
...
async function onlineRankedQueue() {
    ...
    try {
        ...
        enterRoom(result);
        if (!result.matched)
            onlineLobbyStatus.textContent = `Waiting for a ranked opponent near your level. Room ${result.code}.`;
    }
    catch (e) {
        onlineLobbyStatus.textContent = e.message;
    }
...
function enterRoom(info) {
    ...
    resetGameLocal();
    if (info.match?.state?.fen)
        loadFen(info.match.state.fen);
...
function onlineHandleMove(data) {
    if (!online)
        return;
    if (data.by === online.myColor)
        return; // it was our move; already applied locally
...
function onlineHandleGameOver(data) {
    ...
    if (Array.isArray(data.ratings)) {
        ...
        if (mine)
            saveProfile({ token: currentProfile.token, user: mine });
    }
    stopClock();
    renderOnlineMatch();
}
...
async function onlineRematch() {
    if (!online || online.role !== "player" || online.status !== "finished")
        return;
    try {
...
function renderOnlineMatch() {
    ...
    if (online.status === "waiting")
        status = `Waiting for an opponent… share code ${online.code}.`;
    else if (online.status === "playing") {
        if (online.role === "spectator")
            status = `${turn === "red" ? "Red" : "Black"} to move.`;
        else if (online.myColor === turn)
            status = "Your turn.";
        else
    ...
    if (online.ranked && status)
        status = `Rank match · ${status}`;
...
window.addEventListener("keydown", e => {
    if (e.key === "Escape" && !onlineOverlay.hidden)
        closeMatchOverlay();
});
...
window.addEventListener("beforeunload", () => {
    if (online) {
        ...
        navigator.sendBeacon?.(`${base}/api/match/${encodeURIComponent(online.code)}/leave`, new Blob([JSON.stringify({ playerId: online.playerId })], { type: "application/json" }));
    }
});
...
function pgnParseText(text) {
    ...
    for (const raw of tokens) {
        if (/^\d+\.+$/.test(raw))
            continue; // move numbers like "1." or "1..."
        if (/^(1-0|0-1|1\/2-1\/2|\*)$/.test(raw))
            break; // result terminator
...
async function renderPgnLibraryResults() {
    ...
    try {
        ...
        if (requestId !== pgnLibrarySearchRequest)
            return;
...
async function pgnComputeAnalysis(targetIndex, beforeFen, beforeBoard, mover, playedUci) {
    if (pgnAnalysisCache.has(targetIndex))
        return;
    ...
    if (pgnIndex === targetIndex && modeSelect.value === "pgn")
        pgnRenderAnalysis();
}
...
pgnOverlay.addEventListener("click", e => {
    if (e.target === pgnOverlay)
        closePgnOverlay();
});
pgnLoadBtn.addEventListener("click", () => {
    if (pgnLoadText(pgnInput.value))
        closePgnOverlay();
});
...
pgnLibraryResults.addEventListener("click", e => {
    ...
    if (btn?.dataset.id)
        openLibraryGame(btn.dataset.id);
});
...
window.addEventListener("keydown", e => {
    if (modeSelect.value !== "pgn" || pgnNav.hidden)
        return;
