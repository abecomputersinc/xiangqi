...
const pvLine = document.querySelector("#pvLine");
const moveList = document.querySelector("#moveList");
const fenText = document.querySelector("#fenText");
const fenCopyBtn = document.querySelector("#fenCopyBtn");

// Clock DOM
const clockRowRed = document.querySelector("#clockRowRed");
const clockRowBlack = document.querySelector("#clockRowBlack");
const redClockBase = document.querySelector("#redClockBase");
const redClockMove = document.querySelector("#redClockMove");
const blackClockBase = document.querySelector("#blackClockBase");
const blackClockMove = document.querySelector("#blackClockMove");

// PGN viewer DOM
const pgnOverlay = document.querySelector("#pgnOverlay");
const pgnOverlayClose = document.querySelector("#pgnOverlayClose");
const pgnInput = document.querySelector("#pgnInput");
const pgnFile = document.querySelector("#pgnFile");
const pgnLoadBtn = document.querySelector("#pgnLoadBtn");
const pgnStatus = document.querySelector("#pgnStatus");
const pgnLibraryPanel = document.querySelector("#pgnLibraryPanel");
const pgnSearchInput = document.querySelector("#pgnSearchInput");
const pgnOpeningSelect = document.querySelector("#pgnOpeningSelect");
const pgnLibraryCount = document.querySelector("#pgnLibraryCount");
const pgnLibraryResults = document.querySelector("#pgnLibraryResults");
const pgnNav = document.querySelector("#pgnNav");
const pgnFirstBtn = document.querySelector("#pgnFirstBtn");
const pgnPrevBtn = document.querySelector("#pgnPrevBtn");
const pgnNextBtn = document.querySelector("#pgnNextBtn");
const pgnLastBtn = document.querySelector("#pgnLastBtn");
const pgnPosition = document.querySelector("#pgnPosition");
const pgnLastMoveEl = document.querySelector("#pgnLastMove");
const pgnImportAgain = document.querySelector("#pgnImportAgain");

// Startup mode picker DOM
const startupOverlay = document.querySelector("#startupOverlay");
const startupModeStep = document.querySelector("#startupModeStep");
const startupSideStep = document.querySelector("#startupSideStep");
const startupSideBack = document.querySelector("#startupSideBack");
const startupAuthActions = document.querySelector("#startupAuthActions");
const startupLoginBtn = document.querySelector("#startupLoginBtn");
const startupSignupBtn = document.querySelector("#startupSignupBtn");
const startupOptions = document.querySelectorAll(".startup-option:not(.startup-side-option)");
const startupSideOptions = document.querySelectorAll(".startup-side-option");

// Local profile DOM
const profilePanel = document.querySelector(".profile-panel");
const profileNameEl = document.querySelector("#profileName");
const profileLevelEl = document.querySelector("#profileLevel");
...
const onlineOverlay = document.querySelector("#onlineOverlay");
...
const onlineMatchEl = document.querySelector("#onlineMatch");
const onlineNickname = document.querySelector("#onlineNickname");
const onlineCreateBtn = document.querySelector("#onlineCreateBtn");
const onlineQueueBtn = document.querySelector("#onlineQueueBtn");
const onlineRankedQueueBtn = document.querySelector("#onlineRankedQueueBtn");
const onlineJoinCode = document.querySelector("#onlineJoinCode");
const onlineJoinBtn = document.querySelector("#onlineJoinBtn");
const onlineSpectateBtn = document.querySelector("#onlineSpectateBtn");
const onlineLobbyStatus = document.querySelector("#onlineLobbyStatus");
const onlineRoomCode = document.querySelector("#onlineRoomCode");
const onlineCopyCode = document.querySelector("#onlineCopyCode");
const onlineLeaveBtn = document.querySelector("#onlineLeaveBtn");
const onlinePlayerRed = document.querySelector("#onlinePlayerRed");
const onlinePlayerBlack = document.querySelector("#onlinePlayerBlack");
const onlineMatchStatus = document.querySelector("#onlineMatchStatus");
const onlineRematchBtn = document.querySelector("#onlineRematchBtn");
const onlineResignBtn = document.querySelector("#onlineResignBtn");
const onlineSpectators = document.querySelector("#onlineSpectators");
...
function recordToUci(record) {
  return `${squareToUci(record.fromR, record.fromC)}${squareToUci(record.toR, record.toC)}`;
}
...
function buildPgn() {
  ...
  const headers = [
    ["Event", online?.ranked ? "Rank Match" : modeSelect.value === "online" ? "Online Match" : "Xiangqi Game"],
...
async function makeMove(fromR, fromC, toR, toC, source = "human") {
  ...
  const liveRec = wantsRecommendation && !cachedRec
    ? enginePost("/api/topmoves", { fen: beforeFen, count: 3 }).catch(() => null)
  ...
  if (!cachedRec && best && wantsRecommendation) {
    ...
    showRecommendations(moves, beforeBoard, mover, playedUci);
  }
...
function showRecommendations(moves, beforeBoard, mover, playedUci) {
  ...
  lastUserRecommendationText = recLines.length ? `Recommendations:\n${recLines.join("\n")}` : "";
...
function triggerPreMoveAnalysis() {
  if (modeSelect.value === "pgn") return;        // PGN mode has its own cache
  ...
  enginePost("/api/topmoves", { fen, count: 3 })
    .then(result => {
...
async function openEventStream() {
  ...
  es.addEventListener("joined", e => onlineHandleJoined(JSON.parse(e.data)));
  es.addEventListener("left", e => onlineHandleLeft(JSON.parse(e.data)));
...
modeSelect.addEventListener("change", () => {
  ...
  // Hide the clock section in PGN viewer mode (replay has no time pressure).
  document.querySelector("#clocks").hidden = isPgn;
  if (!isOnline && online) {
    onlineLeave();
    closeMatchOverlay();
...
async function pgnComputeAnalysis(targetIndex, beforeFen, beforeBoard, mover, playedUci) {
  ...
  try {
    const result = await enginePost("/api/topmoves", { fen: beforeFen, depth: Number(depthInput.value) + 1, count: 3 });
...
for (const btn of startupOptions) {
  btn.addEventListener("click", () => {
    ...
    if (!mode) return;
    if (mode === "ai") {
      // Two-step: show side picker before starting.
